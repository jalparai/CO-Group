...
const fetchSections = async () => {
  try {
    const response = await apiclient.get("/sections/nav-bar");
    console.log("ebhhebfbfu")
...
export default function Navbar() {
  ...
  useEffect(() => {
    const fetchSectionsData = async () => {
      ...
      setProductsMenu(sectionsData.filter((section) => section?.title === "Products") || []);
      setSolutionsMenu(sectionsData.filter((section) => section?.title === "Solutions") || []);
      setExploreMenu(sectionsData.filter((section) => section?.title === "Explore") || []);
      setCTA(sectionsData.find((section) => section?.title === "Call To Action") || []);
      setBanner(sectionsData.find((section) => section?.title === "banner") || []);
    }
    fetchSectionsData();
  }
  , []);
 ...
 <>
   ...
   <div className='bg-[#FF0000]'>
    <div className='mx-auto w-full max-w-[1271px]  top-0 flex justify-between items-center  p-3'>
      <div className='flex justify-between w-[90%] lg:w-[100%] items-center gap-4'>
        <h2 className='lg:text-[20px] text-[12px] text-white font-bold'>{banner?.generalSection?.description || "Book a Free Demo Call Now!"}</h2>
        <a href={banner?.generalSection?.buttons[0].url || "https://lp.codpowergroup.com/book-demo-call"} 
            target={banner?.generalSection?.buttons[0].target || "_black"} className='text-white block w-fit text-[10px] lg:text-[16px] rounded-full py-2 px-2 lg:px-4 border border-white'>
          {banner?.generalSection?.buttons[0].text || "Book a Free Demo"}
        </a>
      </div>
      <button 
        className='text-white text-xl lg:text-[32px] font-bold  relative lg:left-[90px] left-0'
        onClick={() => setIsVisibleBanner(false)}
      >
        ×
      </button>
    </div>
    ...
    <header
    className={`top-auto left-0 right-0 border-b border-[#373737] lg:py-5 z-[3000] backdrop-blur-lg 
      ...
      <div className="mx-auto w-full max-w-[1271px] px-4 lg:px-8 h-full">
        ...
        >
            ...
            <div className="hidden lg:flex lg:gap-x-8 ml-[64px] h-full items-center">
              ...
              >
                ...
                {isProductsMenuOpen && (
                  ...
                  >
                    <ProductsMenu isMenuOpen={isProductsMenuOpen} data={productsMenu}/>
                  </div>
                )}
              </div>
              ...
              >
                ...
                {isSolutionsMenuOpen && (
                  ...
                  >
                    <SolutionsMenu data={solutionsMenu} />
                  </div>
                )}
              </div>
          ...
          <div className="hidden lg:flex lg:flex-1 lg:justify-end space-x-4 h-full items-center">
            ...
            >
              <img src={Support} alt="Support" />{cta?.generalSection?.buttons[0]?.text || "Contact Sales"}
            </Link>
            <motion.div
              initial={{
            ...
            >
              <a
                href={cta?.generalSection?.buttons[1]?.url || "https://get-started.codpowergroup.com/registration"}
                target="blank"
              ...
              >
                {cta?.generalSection?.buttons[1]?.text || `Get Started${" "}`}
                <svg
                  width="13"
                  height="13"
                  viewBox="0 0 15 14"
